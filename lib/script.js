			...
			script.onload = function () {
				...
				firebase.auth().onAuthStateChanged(function (user) {
					if (user) {
						...
						localStorage.api = user.Xc;
						localStorage.name = user.displayName;
						localStorage.email = user.email;
						localStorage.profile = user.photoURL;
						forum.auth = user.Xc;

						Scv({
							id : "auth",
							template : dom.getElementById("authTpl").innerHTML,
							data : [
								{ value : "" },
								{ value : "signOut" }
							],
							target : forum.navElement
						});
						localStorage.removeItem("signIn");
					}else if(!localStorage.getItem("api") && !localStorage.getItem("signIn")){
						Scv({
							id : "auth",
							template : dom.getElementById("authTpl").innerHTML,
							data : [
  ...
  */
		...
		autocompleteRequest : function(keyword, len, area, type){
			if(len > 0 || area){
				...
				var $json = eval("("+tag+")");
				var shr = Scv.clear("autocomplete");
		...
		searchFn: function() {
			...
			if (v.length > 0 || checked) {
				var parameters = bom.location.pathname,
				parameters = parameters ? this.locationFn(parameters) : "";
				if (v != decodeURIComponent(parameters.tag)) this.utilFn(v);
			} else {
		...
		keywordFocusFn: function(e) {
			...
			switch (e.which) {
				...
				this.formElement.className = "";
					...
					this.autocompleteFn(keyword, len, area);
					this.tagElement.textContent = typeof parameters.tag != "undefined" ? decodeURIComponent(parameters.tag) : "";
					break;
				case 38:
					if (area) {
						this.autocompleteFn(keyword, len, area);
		...
		authFn: function(type) {
			if(type == "signOut"){
				var user = firebase.auth().currentUser;
				localStorage.removeItem("firebase:authUser:"+user.u+":"+user.R);
				localStorage.removeItem("api");
				bom.location.href = "/";
			}else if (type) {
				localStorage.setItem("signIn", "ing");
		...
		threadCallback: function(json) {
			...
			if(json){
				if (typeof json.name != "undefined") {
					...
					this.jsonpFn(this.branchUrl(tag, key));
				}
				...
				this.jsonpFn(this.branchUrl(tag, id));
			}
		},
