(function (global, factory) {
	...
	function setItem(option){
		setTimeout(function(){
			...
			if(oldValue != _newValue){
				typeof newValue.state == "undefined" ? newValue.state = { type : "set" } : newValue.state.type = "set";
	...
	function removeItem(option){
		$tore.removeItem(option.id+"-!#"+option.idx);
	}
	...
	function clear(id){
		...
		for(var i = 0, len = item.length; i < len; i++){
			Scv.removeItem({id : id, idx : item[i]});
			items.push(Scv.getItem({id : id, idx : item[i]}));
		}
		return items;
	}
